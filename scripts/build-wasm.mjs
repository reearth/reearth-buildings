#!/usr/bin/env node
// Build all Rust crates that ship as WASM, then patch the wasm-bindgen
// glue to be compatible with the workerd runtime.
//
// Cloudflare requires `.wasm` imports to be resolved as
// `WebAssembly.Module` rather than the auto-instantiated namespace that
// wasm-pack assumes when run with `--target bundler`. We rewrite the
// main glue file accordingly:
...
const crates = [
  // Single WASM entry point — buildings-wasm bundles the mvt /
  // terrain / buildings-core crates internally.
  { name: "buildings-wasm", snake: "buildings_wasm" },
];
...
function optimizeWasm(outDir, snake) {
  ...
  // Enable wasm proposals modern rustc emits by default. Without these
  // flags older binaryen (e.g. the Ubuntu LTS apt package) bails with
...
function patchGlue(outDir, snake) {
  ...
  const patched = `// Patched by scripts/build-wasm.mjs for the workerd runtime.
